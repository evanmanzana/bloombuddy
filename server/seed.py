...
def seed_users():
    ...
    for _ in range(5):
        fake_img_url = fake.image_url(
            width=200, height=200
        )  # Generate a fake image URL with desired width and height
    ...
    with app.app_context():
        for user in users:
            ...
            for plant in random_plants:
                ...
                # Create the UserPlant record and add the CareTask object to the relationship
                user_plant = UserPlant(
                    user_id=user.id,
                    plant_id=plant.id,
                )
                user_plant.caretask.append(
                    care_task
                )  # Add the CareTask to the relationship
                db.session.add(user_plant)

            db.session.commit()

        print("Seeding user plants completed successfully.")


def seed_care_tasks():
    user_plants = UserPlant.query.all()
